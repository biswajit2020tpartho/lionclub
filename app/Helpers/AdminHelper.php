...
class AdminHelper
{
     ...
     public static function getEmailSetting($name)
    {
        ...
        $query = DB::table('email_settings')->select($name)->first();
        if(!empty($query))
        {
            Cache::forever('setting_'.$name, $query->$name);
    ...
    {
        $menu_active = DB::table('admin_menus')->whereRaw("admin_menus.id IN (select id_admin_menus from admin_privileges_roles where    id_admin_privileges = '".self::myPrivilegeId()."')")->where('parent_id', 0)->where('is_active', 1)->orderby('sorting', 'asc')->select('admin_menus.*')->get();
        ...
        foreach ($menu_active as &$menu) {
            ...
            $child = DB::table('admin_menus')->whereRaw("admin_menus.id IN (select id_admin_menus from admin_privileges_roles where id_admin_privileges = '".self::myPrivilegeId()."')")->where('is_active', 1)->where('parent_id', $menu->id)->select('admin_menus.*')->orderby('sorting', 'asc')->get();
            if (count($child)) {
    ...
    public static function sendEmail($config = [])
    {
        ...
        /*\Config::set('mail.driver', self::getEmailSetting('mail_driver'));
        \Config::set('mail.host', self::getEmailSetting('smtp_host'));
        \Config::set('mail.port', self::getEmailSetting('smtp_port'));
        \Config::set('mail.username', self::getEmailSetting('smtp_username'));
        \Config::set('mail.password', self::getEmailSetting('smtp_password'));
        \Config::set('mail.encription', 'tls');*/

        /*$emailBody = '<p>This is a test eamil</p>';
        $eamilcontent = array(
            'emailBody'=> $emailBody,
            );
        $fromName = 'Homeopathy';
        $fromEmail = 'test.developers2017@gmail.com';
        $devuser = 'test.developers2020@gmail.com';
        $emailId = 'test.developers2020@gmail.com';
        $mailTitle = 'Homeopaty Example'; 
        $replyTo = 'test.developers2021@gmail.com';
        $result = \Mail::send(['html'=>'admin.emails.email_template'], $eamilcontent, function($message) use ($fromName, $fromEmail, $devuser, $emailId, $mailTitle, $replyTo) {
            $message->to($emailId)
        ...
        \Config::set('mail.username', self::getEmailSetting('smtp_username'));
        \Config::set('mail.password', self::getEmailSetting('smtp_password'));
        \Config::set('mail.encription', 'ssl');
         //dd(\Config::all());*/
        $to = $config['to'];
        $data = $config['data'];
        ...
        if (!empty($config['send_at'])) {
            ...
            $a['email_from_email'] = $template->from_email ?: AdminHelper::getSetting('email_sender');
            $a['email_from_name'] = $template->from_name ?: AdminHelper::getSetting('appname');
    ...
    public static function generateMasterSlots($slot_duration,$day_start,$day_end,$night_start,$night_end)
    {
        //dd($slot_duration);
        $start_time = date('Y-m-d ').$day_start;
        $end_time = date('Y-m-d ').$day_end;
        ...
        $out_day = array();   //array of slots 
        for($i = $start; $i<$end;)  //for loop 
        {
            $time1 = $i->format('H:i:s');   //take hour and minute
            $i->modify("+".$slot_duration." minutes");      //add 20 minutes
            $time2 = $i->format('H:i:s');     //take hour and minute
            $slot = $time1."-".$time2;      //create a format 12:40-13:00 etc
            if($i<=$end)  //if less than end time
            {
                $count++;           //add count
                $slots = ['start'=>$time1, 'stop'=>$time2];         //add count
                array_push($out_day,$slots); //add slot to array
            }
        }
        $start_time = date('Y-m-d ').$night_start;
        $end_time = date('Y-m-d ').$night_end;
        $start = DateTime::createFromFormat('Y-m-d H:i:s',$start_time);  //create date time objects
        $end = DateTime::createFromFormat('Y-m-d H:i:s',$end_time);  //create date time objects
        $count = 0;  //number of slots
        $out_night = array();   //array of slots 
        for($i = $start; $i<=$end;)  //for loop 
        {
            $time1 = $i->format('H:i:s');   //take hour and minute
            $i->modify("+".$slot_duration." minutes");      //add 20 minutes
            ...
            if($i<=$end)  //if less than end time
            {
                $count++;           //add count
                $slots = ['start'=>$time1, 'stop'=>$time2];         //add count
                array_push($out_night,$slots); //add slot to array
            }
        }
        //dd($out_day);
        
        foreach($out_day as $key=>$val)
        {
            MasterSlot::insert([
                'slot_label' => 'day',                             
                'slot_from' => $val['start'],                              
        ...
        foreach($out_night as $keyn=>$valn)
        {
            MasterSlot::insert([
                'slot_label' => 'night',                             
                'slot_from' => $valn['start'],                              
                'slot_to' => $valn['stop'],                                                               
                'created_at' => date('Y-m-d H:i:s')
            ]);
        }
...
//         }
//         $table = AdminHelper::parseSqlTable($table);
...
//     }
    ...
    {
        $key = AdminHelper::findPrimaryKey($table);
    ...
    {
        $parent_table = AdminHelper::parseSqlTable($parent_table)['table'];
        $child_table = AdminHelper::parseSqlTable($child_table)['table'];
    ...
    public static function getUrlParameters($exception = null)
    {
        ...
        if ($get) {
            ...
            foreach ($string_parameters_array as $s) {
                $part = explode('=', $s);
                $name = (!empty($part) && !empty($part[0]))?urldecode($part[0]):'';
                $value = (!empty($part) && !empty($part[1]))?urldecode($part[1]):'';
                if ($name) {
    ...
    public static function sendNotification($config = [])
    {
        ...
        $to = $config['to'];
        $parent_id = (!empty($config['parent_id']))?$config['parent_id']:'';
        $id_cms_users = (!empty($config['id_cms_users']))?$config['id_cms_users']:[];
        $id_cms_users = (!empty($id_cms_users)) ?$id_cms_users: [AdminHelper::myId()];
    ...
    public static function sendFCM($regID = [], $data=null)
    {
        ...
        $apikey = AdminHelper::getSetting('google_fcm_key');
    ...
    public static function generateController($table, $name = null)
    {
        ...
        $coloms = AdminHelper::getTableColumns($table);
        $name_col = AdminHelper::getNameTable($coloms);
        $pk = CB::pk($table);
...
<?php namespace App\Http\Controllers;
    ...
    ';
        ...
        foreach ($coloms_col as $c) {
            ...
            if (substr($field, 0, 3) == 'id_') {
                ...
                $joincols = AdminHelper::getTableColumns($jointable);
                $joinname = AdminHelper::getNameTable($joincols);
        ...
        foreach ($coloms as $c) {
            ...
            $typedata = AdminHelper::getFieldType($table, $field);
    ...
    */
    public static function routeController($prefix, $controller, $namespace = null)
    {
        ...
        $namespace = (!empty($namespace)) ?$namespace: 'App\Http\Controllers';
    ...
    // checkings
    public static function checkCategoryAllocation($id)
    {
        $allocation_design = MasterDesign::where('category_id',$id)->count();
        return ($allocation_design>0) ? 0 :1;
        
        
       
    }
    
    public static function checkServiceAllocation($id)
    {
        $allocation_home = HomepageSetting::whereRaw("find_in_set($id,section_2_services)")->orWhereRaw("find_in_set($id,section_4_services)")->orWhereRaw("find_in_set($id,section_5_services)")->count();
        if($allocation_home>0)
        {
            return 0;
        }
        else
        {
                $allocation_service_consultant = ConsultantService::where('service_id',$id)->count();
                if($allocation_service_consultant>0)
                {
                    return 0;
                }
                else
                {
                    $allocation_booking_consultant = Booking::where('service_id',$id)->count();
                    if($allocation_booking_consultant>0)
                    {
                        return 0;
                    }
                    else{
    ...
    //Rahul
    public static function check_country_allocations($id)
    {
        $allocation_user = User::where('country_id',$id)->count();
        if($allocation_user>0)
        {
            return 0;

        }
        else{
            $allocation_user_profile = UserProfile::where('store_country',$id)->count();
            if($allocation_user_profile>0)
            {
                return 0;
    
            }
            else{
                $allocation_user_address = UserAddress::where('country_id',$id)->count();
                if($allocation_user_address>0)
                {
                    return 0;
        
                }
                else{
                    $allocation_order = Order::where('billing_country_id',$id)->orWhere('shipping_country_id',$id)->count();
                    ...
                    {
                        $allocation_state = State::where('country_id',$id)->count();
                        if($allocation_state>0)
                        {
                            return 0;
    ...
    public static function check_state_allocations($id)
    {
        $allocation_user = User::where('state_id',$id)->count();
        if($allocation_user>0)
        {
            return 0;
            ...
            $allocation_user_profile = UserProfile::where('store_state',$id)->count();
            if($allocation_user_profile>0)
            {
                return 0;
                ...
                $allocation_user_address = UserAddress::where('state_id',$id)->count();
                if($allocation_user_address>0)
                {
                    return 0;
                    ...
                    $allocation_order = Order::where('billing_state_id',$id)->orWhere('shipping_state_id',$id)->count();
                    if($allocation_order>0)
                    {
                        return 0;
                    ...
                    {
                        $allocation_city = City::where('state_id',$id)->count();
                        if($allocation_city>0)
                        {
                            return 0;
    ...
    public static function check_city_allocations($id)
    {
        $allocation_user = User::where('city_id',$id)->count();
        if($allocation_user>0)
        {
            return 0;
            ...
            $allocation_user_profile = UserProfile::where('store_city',$id)->count();
            if($allocation_user_profile>0)
            {
                return 0;
                ...
                $allocation_user_address = UserAddress::where('city_id',$id)->count();
                if($allocation_user_address>0)
                {
                    return 0;
                    ...
                    $city_dtls = City::where('id',$id)->first();
                    $allocation_order = Order::where('billing_city',$city_dtls->name)->orWhere('shipping_city',$city_dtls->name)->count();
                    if($allocation_order>0)
                    {
                        return 0;
    ...
    public static function checkThemeAllocation($id)
    {
        $allocation_design = MasterDesign::where('theme_id',$id)->count();
        if($allocation_design>0)
        {
            return 0;
        }
        else
        {
            $allocation_order_designs = OrderDesign::where('theme_id',$id)->count();
            return ($allocation_order_designs>0) ? 0 :1;
            
        }
        
       
    }
    public static function checkColorAllocation($id)
    {
        $allocation_design = MasterDesign::where('color_id',$id)->count();
        if($allocation_design>0)
        {
            return 0;
        }
        ...
        {
            $allocation_order_designs = OrderDesign::where('color_id',$id)->count();
            return ($allocation_order_designs>0) ? 0 :1;
    ...
    public static function checkAttributeAllocation($id)
    {
        $design_allocations = DesignAttribute::where('attributes_id',$id)->count();
        return $x = ($design_allocations>0) ? 0 : 1;
        
    }
    public static function checkAttributeGroupAllocation($id)
    {
        $design_allocations = DesignAttribute::where('attribute_groups_id',$id)->count();
        if($design_allocations>0)
        {
            return 0;
        }
        else{
            $design_allocations = Attribute::where('attribute_groups_id',$id)->count();
            return $x = ($design_allocations>0) ? 0 : 1;
        }
        
        
    }
    public static function checkOptionAllocation($id)
    {
        $option_allocations = OptionValue::where('option_id',$id)->count();
        return $x = ($option_allocations>0) ? 0 : 1;
    }

    public static function checkWeaveAllocation($id)
    {
        $weave_allocations = FabricOption::where('weave_id',$id)->count();
        return $x = ($weave_allocations>0) ? 0 : 1;
        //return 1;
        
    }
    public static function checkFbricAllocation($id)
    {
        $fabric_allocations = OrderDesign::where('fabric_id',$id)->count();
        return $x = ($fabric_allocations>0) ? 0 : 1;
    }
    public static function checkDesignAllocation($id)
    {
        $design_allocations = OrderDesign::where('design_id',$id)->count();
        return $x = ($design_allocations>0) ? 0 : 1;
    }
    public static function checkCouponAllocation($id)
    {
        $coupon_allocations = CouponUseHistory::where('coupon_id',$id)->count();
        if($coupon_allocations>0)
        {
            return 0;
        }
        else{
            $coupon_allocations_order = Oredr::where('coupon_id',$id)->count();
            return $x = ($coupon_allocations_order>0) ? 0 : 1;
        }
     ...
     /*-------------------------------For Stock Logs-----------------------*/
     public static function insertLogStock($design_id,$description, $stock_type,$qty, $changed_qty,$details = '')
     {
