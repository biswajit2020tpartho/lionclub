...
<div class="list-grid-nav hstack gap-1 mb-3">
    ...
    <a href="{{ route('getAddGroup') }}?return_url={{ route('getManageGroup') }}"
...
<div class="card">
    <div class="card-header align-items-center d-flex">
        ...
        <div class="box-tools pull-right" style="position: relative;margin-top: -5px;margin-right: -10px">   
            ...
            <form method="get" style="display:inline-block;width: 290px;"
                action="{{ route('getManageGroup') }}">
                <div class="input-group">
                    <input type="text" name="q" value="{{ request()->get('q') }}" class="form-control rounded-0 pull-right" placeholder="Search">

                    <div class="input-group-btn">
                    	@if(!empty(request()->get('q')))
                        <button type="button" onclick="location.href='{{ route('getManageGroup') }}'" title="Reset" class="btn rounded-0 btn-warning"><i class="fa fa-ban"></i></button>
                        @endif
    ...
    <div class="card-body">
        <div class="table-responsive">
            <form id="form-table" method="post" action="{{ route('getManageGroup') }}/action-selected">
                ...
                <table id="table_dashboard" class="table align-middle table-nowrap table-hover mb-0">
                    <thead class="table-blue">
                        <tr class="active">
                            ...
                            <th width="auto"><a href="{{ route('getManageGroup') }}?filter_column=name&sorting={{(request()->get('filter_column')=='name' && request()->get('sorting')=='asc')?'desc':'asc'}}" title="Click to sort ascending">Group Name &nbsp; <i class="fa fa-sort"></i></a></th>
                            <th width="auto">Member Category&nbsp;</th>
                    ...
                    <tbody>
                        ...
                        <tr>
                            ...
                            <td>{{ $data->name }}</td>
                            <td>{{ $data->getCategoryDetails->name }}</td>
                           <td>@if($data->status==1) <span class="badge bg-success">Active</span> @else <span class="badge bg-danger">Inactive</span> @endif</td> 
                            <td>
                                <div class="button_action" style="text-align:right">
                                    ...
                                    <a class="btn btn-sm btn-primary btn-detail" title="Detail Data" href="{{ route('getManageGroup') }}/detail/{{$data->id}}?return_url={{ route('getManageGroup') }}"><i class="fa fa-eye"></i></a>
                                    @endif
                                    @if(AdminHelper::isUpdate())
                                    <a class="btn btn-sm btn-success btn-edit" title="Edit Data" href="{{ route('getManageGroup') }}/edit/{{$data->id}}?return_url={{ route('getManageGroup') }}"><i class="fa fa-pencil"></i></a>
                                    @endif
                            ...
                            <a class="btn btn-sm btn-warning btn-delete" title="Delete" href="javascript:;" onclick="Swal.fire({
				                    ...
				                    t.isConfirmed?location.href='{{ AdminHelper::adminpath() }}/manage-group/delete/{{$data->id}}':'' });">
