...
<div class="list-grid-nav hstack gap-1 mb-3">
    ...
    <a href="{{ route('getAddMem') }}?return_url={{ route('getManageMem') }}"
...
<div class="card">
    <div class="card-header align-items-center d-flex">
        ...
        <div class="box-tools pull-right" style="position: relative;margin-top: -5px;margin-right: -10px">   
            ...
            <form method="get" style="display:inline-block;width: 290px;"
                action="{{ route('getManageMem') }}">
                <div class="input-group">
                    <input type="text" name="q" value="{{ request()->get('q') }}" class="form-control rounded-0 pull-right" placeholder="Search">

                    <div class="input-group-btn">
                    	@if(!empty(request()->get('q')))
                        <button type="button" onclick="location.href='{{ route('getManageMem') }}'" title="Reset" class="btn rounded-0 btn-warning"><i class="fa fa-ban"></i></button>
                        @endif
    ...
    <div class="card-body">
        <div class="table-responsive">
            <form id="form-table" method="post" action="{{ route('getManageMem') }}/action-selected">
                ...
                <table id="table_dashboard" class="table align-middle table-nowrap table-hover mb-0">
                    <thead class="table-blue">
                        <tr class="active">
                            ...
                            <th width="auto"><a href="{{ route('getManageMem') }}?filter_column=page_title&sorting={{(request()->get('filter_column')=='page_title' && request()->get('sorting')=='asc')?'desc':'asc'}}" title="Click to sort ascending">Name &nbsp; <i class="fa fa-sort"></i></a></th>
                            <th width="auto"><a href="{{ route('getManageMem') }}?filter_column=name&sorting={{(request()->get('filter_column')=='name' && request()->get('sorting')=='asc')?'desc':'asc'}}" title="Click to sort ascending">Email &nbsp; <i class="fa fa-sort"></i></a></th>
                            <th width="auto"><a href="{{ route('getManageMem') }}?filter_column=mobile&sorting={{(request()->get('filter_column')=='mobile' && request()->get('sorting')=='asc')?'desc':'asc'}}" title="Click to sort ascending">Mobile &nbsp; <i class="fa fa-sort"></i></a></th> 
                             <th width="auto"><a href="{{ route('getManageMem') }}?filter_column=date_of_birth&sorting={{(request()->get('filter_column')=='date_of_birth' && request()->get('sorting')=='asc')?'desc':'asc'}}" title="Click to sort ascending">Date of birth &nbsp; <i class="fa fa-sort"></i></a></th>
                              <th width="auto"><a href="{{ route('getManageMem') }}?filter_column=date_of_weding&sorting={{(request()->get('filter_column')=='date_of_weding' && request()->get('sorting')=='asc')?'desc':'asc'}}" title="Click to sort ascending">Date of weding &nbsp; <i class="fa fa-sort"></i></a></th>             
                    ...
                    <tbody>
                        ...
                        <tr>
                            ...
                            @if(!empty($data->photo) && (Storage::exists($data->photo) || file_exists($data->photo)))
                                <div class="prev-img-thumb" style="width: 50px;"><img src="{{ url($data->photo) }}"></div>
                            @else
                                <div class="prev-img-thumb" style="width: 50px;"><img src="{{ asset('frontend/images/users/user-dummy-img.jpg') }}"></div>
                            @endif   
                            </td>
                            <td>{{ $data->name }}</td>
                            <td>{{ $data->email }}</td>
                           ...
                           <td>@if($data->status==1) <span class="badge bg-success">Active</span> @else <span class="badge bg-danger">Inactive</span> @endif</td> 
                            <td>
                                <div class="button_action" style="text-align:right">
                                    ...
                                    <a class="btn btn-sm btn-primary btn-detail" title="Detail Data" href="{{ route('getManageMem') }}/detail/{{$data->id}}?return_url={{ route('getManageMem') }}"><i class="fa fa-eye"></i></a>
                                    @endif
                                    @if(AdminHelper::isUpdate())
                                    <a class="btn btn-sm btn-success btn-edit" title="Edit Data" href="{{ route('getManageMem') }}/edit/{{$data->id}}?return_url={{ route('getManageMem') }}"><i class="fa fa-pencil"></i></a>
                                    @endif
